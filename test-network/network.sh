...
function printHelp() {
  ...
  echo "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Score and Reservation"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Score Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-score.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create Reservation Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-reservation.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
    ...
    # echo "##########################################################"
    # echo "############ Create Orderer2 Org Identities ###############"
    # echo "##########################################################"
  ...
  echo
  echo "Generate CCP files for Score and Reservation"
...
## call the script to join create the channel and join the peers of score and reservation
...
function networkDown() {
  # stop org3 containers also in addition to score and rseervation, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/score/msp organizations/fabric-ca/score/tls-cert.pem organizations/fabric-ca/score/ca-cert.pem organizations/fabric-ca/score/IssuerPublicKey organizations/fabric-ca/score/IssuerRevocationPublicKey organizations/fabric-ca/score/fabric-ca-server.db'

    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/reservation/msp organizations/fabric-ca/reservation/tls-cert.pem organizations/fabric-ca/reservation/ca-cert.pem organizations/fabric-ca/reservation/IssuerPublicKey organizations/fabric-ca/reservation/IssuerRevocationPublicKey organizations/fabric-ca/reservation/fabric-ca-server.db'
